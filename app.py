...
# CoinCap v3 base (key required via Streamlit Secrets)
...
# Require API key in Streamlit Secrets (Manage app → Settings → Secrets)
...
def http_get(url, params=None, timeout=60):
    """GET with friendly error output."""
...
def fetch_top100():
    # Ask for 200 ranked assets; filter out stables; take first 100
...
st.caption("Data: CoinCap v3 (API key). Daily prices; correlations = Pearson on log-returns. Volatility = rolling realized σ (annualized).")
...
# Fetch prices with a progress bar
...
# Coverage filter over the correlation window
